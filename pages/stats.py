...
def create_overall_statistics_display(stats):
    ...
    # Add total downtime statistics as table
    if stats.get('total_downtime_minutes', 0) > 0:
        # Calculate percentage downtime
        total_recording_hours = stats.get('total_recording_minutes', 0) / 60
        total_downtime_hours = stats.get('total_downtime_hours', 0)
        
        # Calculate percentages for different time periods
        # Total percentage: downtime / total recording time (entire recorded period)
        downtime_percentage_total = (total_downtime_hours / total_recording_hours * 100) if total_recording_hours > 0 else 0
        
        # Daily percentage: total downtime / 24 hours (1440 minutes)
        downtime_percentage_day = (stats.get('total_downtime_minutes', 0) / 1440 * 100) if stats.get('total_downtime_minutes', 0) > 0 else 0
        
        # Weekly percentage: total downtime / (24 * 7) hours (10080 minutes)
        downtime_percentage_week = (stats.get('total_downtime_minutes', 0) / 10080 * 100) if stats.get('total_downtime_minutes', 0) > 0 else 0
        
        # Yearly percentage: total downtime / (24 * 365.25) hours (525960 minutes)
        downtime_percentage_year = (stats.get('total_downtime_minutes', 0) / 525960 * 100) if stats.get('total_downtime_minutes', 0) > 0 else 0
        
        children.append(
            html.Div(className='total-downtime', children=[
                html.H4('🔴 Summierte Ausfallzeiten aller CIs'),
                html.Table(className='downtime-table', children=[
                    html.Thead([
                        html.Tr([
                            html.Th('Zeitraum'),
                            html.Th('Ausfallzeit'),
                            html.Th('Prozentuale Ausfallzeit')
                        ])
                    ]),
                    html.Tbody([
                        html.Tr([
                            html.Td('Gesamt'),
                            html.Td(format_duration(stats["total_downtime_hours"])),
                            html.Td(f'{downtime_percentage_total:.3f}%' if downtime_percentage_total >= 0.001 else f'{downtime_percentage_total:.6f}%')
                        ]),
                        html.Tr([
                            html.Td('⌀ pro Tag'),
                            html.Td(f'{stats["total_downtime_days"]:.3f} Tage' if stats["total_downtime_days"] >= 0.001 else f'{stats["total_downtime_days"]:.6f} Tage'),
                            html.Td(f'{downtime_percentage_day:.3f}%' if downtime_percentage_day >= 0.001 else f'{downtime_percentage_day:.6f}%')
                        ]),
                        html.Tr([
                            html.Td('⌀ pro Woche'),
                            html.Td(f'{stats["total_downtime_weeks"]:.3f} Wochen' if stats["total_downtime_weeks"] >= 0.001 else f'{stats["total_downtime_weeks"]:.6f} Wochen'),
                            html.Td(f'{downtime_percentage_week:.3f}%' if downtime_percentage_week >= 0.001 else f'{downtime_percentage_week:.6f}%')
                        ]),
                        html.Tr([
                            html.Td('⌀ pro Jahr'),
                            html.Td(f'{stats["total_downtime_years"]:.4f} Jahre' if stats["total_downtime_years"] >= 0.001 else f'{stats["total_downtime_years"]:.6f} Jahre'),
                            html.Td(f'{downtime_percentage_year:.4f}%' if downtime_percentage_year >= 0.001 else f'{downtime_percentage_year:.6f}%')
                        ])
                    ])
                ])
            ])
